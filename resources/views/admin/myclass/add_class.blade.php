        ...
        <div class="row justify-content-center">
            ...
            <div class="col-md-9">
               ...
               <div class="card">
                   ...
                   <div class="card-body">
                       <form action="@if(!empty($editmyclass)){{route('myclass.update',$editmyclass->id)}} @else {{route('myclass.store')}} @endif" method="post">
                           {{csrf_field()}}
                           @if(!empty($editmyclass))   @method('PUT') @endif

                           <div class="form-group">
                               <label>Class Name</label>
                               <input type="text" name="myclass_name" value="@if(!empty($editmyclass)){{$editmyclass->myclass_name}}@endif" class="form-control">
                           </div>
                           <div class="form-group">
                               <label></label>
                               <input type="submit" name="btn" class="btn btn-info" value="@if(!empty($editmyclass)) Update @else Save @endif">
                           </div>
                       </form>
